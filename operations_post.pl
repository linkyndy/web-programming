...
# popup menu
print $query->h1('Alegere operatia');
print $query->popup_menu({-name=>'PopupOne',
			  -values=>['Adunare', 'Scadere',
				    'Inmultire', 'Impartire', 'Ridicare la putere' ],
			  -defaults=>'Adunare'});
print $query->br();
...
if ($query->param()) {
  ...
  my $textInput1 = $query->param(-name=>'TextfieldOne');
	...
	my $operatie = $query->param(-name=>'PopupOne');
    print $query->hr();
    print $query->p('Selectie popup:'), $query->br();
